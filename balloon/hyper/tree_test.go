...
func TestAdd(t *testing.T) {
	...
	for i, c := range testCases {
		...
		err = tree.store.Mutate(mutations, util.Uint64AsBytes(version))
...
func TestAddBulk(t *testing.T) {
	...
	for i, c := range testCases {
		...
		err = tree.store.Mutate(mutations, util.Uint64AsBytes(uint64(i)))
...
func TestConsistencyBetweenAddAndAddBulk(t *testing.T) {
	...
	for i, c := range testCases {
		...
		for j, _ := range c.eventDigests {
			...
			require.NoErrorf(t, addTree.store.Mutate(mutations, util.Uint64AsBytes(uint64(i))), "Error inserting mutations in test %d", j)
		...
		require.NoErrorf(t, err, "This should not fail in test %d", i)
		require.NoErrorf(t, addBulkTree.store.Mutate(mutations, util.Uint64AsBytes(uint64(i))), "Error inserting mutations in test %d", i)
...
func TestAddAndVerify(t *testing.T) {
	...
	for i, c := range testCases {
		...
		_ = tree.store.Mutate(mutations, util.Uint64AsBytes(uint64(i)))
...
func TestDeterministicAdd(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		_ = store1.Mutate(m1, util.Uint64AsBytes(uint64(i)))
		_, m2, _ := tree2.Add(eventDigest, version)
		_ = store2.Mutate(m2, util.Uint64AsBytes(uint64(i)))
	}
...
func TestRebuildCache(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		_ = store.Mutate(mutations, util.Uint64AsBytes(uint64(i)))
	}
