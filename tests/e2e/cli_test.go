	...
	let(t, "Add one event through cli and verify it", func(t *testing.T) {
		...
		let(t, "Add event", func(t *testing.T) {
			...
			cmd := exec.Command("go",
				...
				"client",
				"--api-key=APIKey",
				"--endpoints=https://127.0.0.1:8801",
				"add",
				"--event='test event'",
...
func TestClientToClusterWithLeaderChange(t *testing.T) {
	b0, a0 := newServerSetup(1, true)
	b1, a1 := newServerSetup(2, true)
	b2, a2 := newServerSetup(3, true)
	let, report := spec.New()
	defer func() {
		// a0()
		a1()
	...
	let(t, "Add one event through cli and verify it", func(t *testing.T) {
		let(t, "Add event", func(t *testing.T) {
			...
			cmd := exec.Command("go",
				...
				"client",
				"--api-key=APIKey",
				"--endpoints=https://127.0.0.1:8801,https://127.0.0.1:8802,https://127.0.0.1:8803",
				"add",
				"--event='test event'",
				"--attempt-to-revive-endpoints",
				"--max-retries=3",
